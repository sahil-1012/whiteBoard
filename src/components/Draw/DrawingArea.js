...
const DrawingArea = () => {
  ...
  const handleAddNote = (w, h, shape, x = 200, y = 100) => {
    setNotes([
      ...notes,
      {
        x: x,
  ...
  const handleAddNoteCanvas = (e) => {
    if (!stickyMode)
      return

    const initialWidth = 200
    const initialHeight = 300
    const centeredX = e.target.pointerPos?.['x'] - (initialWidth / 2);
    const centeredY = e.target.pointerPos?.['y'] - (initialHeight / 2);

    handleAddNote(initialWidth, initialHeight, 'Rectangle', centeredX, centeredY, stickyMode);
  };
